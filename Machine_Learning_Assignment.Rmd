...
```{r}
setwd("C:/Users/ADMIN/Desktop/Data_Analytics/Machine Learning/Project")
...
### Data cleaning
Deleting coloumns of missing values:
```{r}
training <- training[, colSums(is.na(training)) == 0]
testing <- testing[, colSums(is.na(testing)) == 0]
```

We also remove the first seven predictors since these variables have little predicting power
```{r}
...
accuracy_rpart
```
From the confusion matrix, the accuracy rate is 0.5, and so the out-of-sample error rate is 0.5. Using classification tree does not predict the outcome very well.
...
We now use random forests to predict the outcome variable
